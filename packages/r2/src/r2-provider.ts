...
export class R2Provider implements MediaProvider {
    ...
    /**
     * Lazy-loads the AWS S3 SDK (R2 is S3-compatible).
     */
    ...
    getUrl(id: string, _transform?: TransformationOptions): string {
        // R2 doesn't support transformations
        if (this.config.publicUrl) {
            return `${this.config.publicUrl}/${id}`;
        }
        // Default R2 URL format
        return `https://${this.config.bucket}.${this.config.accountId}.r2.cloudflarestorage.com/${id}`;
    }
